...
def get_hh_vacancies(language: list[str]) -> list[str]:
    ...
    hh_api_address = "https://api.hh.ru/vacancies"
    headers = {
    ...
    for page in count(0):
        ...
        response = requests.get(
            hh_api_address, params=payload, headers=headers
        )
...
def get_sj_vacancies(language: str, token: str) -> list:
    ...
    sjob_api_address = 'https://api.superjob.ru/2.0/vacancies/'
...
def combine_hh_salaries():
    ...
    for language in PROGRAMM_LANGUAGES:
        vacancies_response = get_hh_vacancies(language)
        salaries = extract_hh_salary(vacancies_response)
        vacancies_count = len(vacancies_response)
        try:
            average_salary = int(sum(salaries) / len(salaries))
        except ZeroDivisionError:
            average_salary = 0

        salary_by_languages[language] = {'vacancies_found': vacancies_count,
...
def combine_sj_vacancies():
    salary_by_languages = {}
    for language in PROGRAMM_LANGUAGES:
        vacancies_response = get_sj_vacancies(language, token)
        salaries = extract_sj_salary(vacancies_response)
        vacancies_count = len(vacancies_response)
        try:
            average_salary = int(sum(salaries) / len(salaries))
        except ZeroDivisionError:
